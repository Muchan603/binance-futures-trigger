...
Скрипт для автоматического выставления и снятия условных ордеров на **Binance USDT-M Futures** в заданную секунду.

**Тип ордеров:** `TAKE_PROFIT_MARKET` — триггер по цене → исполнение по рынку (не лимитные, без стакана).
...
```
В place_time UTC:
  для каждой монеты параллельно:
    - получить текущую цену (bid/ask)
    - BUY  trigger = цена × (1 - distance%)   ← покупка на откате
    - SELL trigger = цена × (1 + distance%)   ← продажа на росте
    - выставить TAKE_PROFIT_MARKET ордер

В cancel_time UTC:
  снять все выставленные ордера параллельно
```
